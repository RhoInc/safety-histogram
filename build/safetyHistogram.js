...
function onInit() {
    ...
    var allMeasures = d3$1.set(this.raw_data.map(function (m) {
        ...
        var allObservations = _this.raw_data.filter(function (d) {
    ...
    if (catMeasures.length) console.warn(catMeasures.length + ' non-numeric endpoint' + (catMeasures.length > 1 ? 's have' : ' has') + ' been removed: ' + catMeasures.join(', '));
...
function updateXDomain(chart) {
    var xMinSelect = chart.controls.wrap.selectAll('.control-group').filter(function (f) {
        return f.option === 'x.domain[0]';
    }).select('input');

    var xMaxSelect = chart.controls.wrap.selectAll('.control-group').filter(function (f) {
...
function onLayout() {
    ...
    var resetContainer = this.controls.wrap.insert('div', '.control-group:nth-child(3)').classed('control-group x-axis', true).datum({
        type: 'button',
        option: 'x.domain',
        label: 'x-axis:'
    }),
        resetLabel = resetContainer.append('span').attr('class', 'control-label').style('text-align', 'right').text('X-axis:'),
        ...
        chart.controls.wrap.selectAll('.control-group').filter(function (f) {
            return f.option === 'x.domain[0]';
        }).select('input').property('value', chart.config.x.domain[0]);

        chart.controls.wrap.selectAll('.control-group').filter(function (f) {
            return f.option === 'x.domain[1]';
        }).select('input').property('value', chart.config.x.domain[1]);

        chart.draw();
    });

    //x-domain lower limit
    var lowerLimitContainer = this.controls.wrap.insert('div', '.control-group:nth-child(4)').classed('control-group x-axis', true).datum({
        type: 'number',
        option: 'x.domain[0]',
        label: 'Lower Limit'
    }),
        lowerLimitLabel = lowerLimitContainer.append('span').attr('class', 'control-label').style('text-align', 'right').text('Lower Limit'),
        lowerLimitControl = lowerLimitContainer.append('input').on('change', updateLimits);

    //x-domain upper limit
    var upperLimitContainer = this.controls.wrap.insert('div', '.control-group:nth-child(5)').classed('control-group x-axis', true).datum({
        type: 'number',
        option: 'x.domain[1]',
        label: 'Upper Limit'
    }),
        upperLimitLabel = upperLimitContainer.append('span').attr('class', 'control-label').style('text-align', 'right').text('Upper Limit'),
...
function onPreprocess() {
    ...
    if (changedMeasureFlag) {
        ...
        this.controls.wrap.selectAll('.x-axis').property('title', 'Initial Limits: [' + this.config.x.domain[0] + ' - ' + this.config.x.domain[1] + ']');

        //Set x-axis domain controls.
        this.controls.wrap.selectAll('.control-group').filter(function (f) {
            return f.option === 'x.domain[0]';
        }).select('input').property('value', this.config.x.domain[0]);
        this.controls.wrap.selectAll('.control-group').filter(function (f) {
            return f.option === 'x.domain[1]';
        }).select('input').property('value', this.config.x.domain[1]);
    }
    ...
    updateXDomain(chart);
    this.raw_data = this.super_raw_data.filter(function (d) {
        return d[chart.config.measure_col] === chart.currentMeasure;
    }).filter(function (f) {
    ...
    var raw_range = d3$1.extent(this.measure_data, function (d) {
        return +d[config.value_col];
    }).map(function (f) {
        return '' + f;
    });
    var full_range_covered = +chart.x_dom[0] == +raw_range[0] && +chart.x_dom[1] == +raw_range[1];
    chart.controls.wrap.selectAll('.control-group').filter(function (f) {
        return f.option === 'x.domain';
    }).select('button').property('disabled', full_range_covered);
}
...
function onDraw() {
    ...
    if (this.config.x.type === 'linear' && this.x_dom[0] === this.x_dom[1]) this.x_dom = [this.x_dom[0] - this.x_dom[0] * 0.05, this.x_dom[1] + this.x_dom[1] * 0.05];
...
function drawNormalRanges(chart) {
    ...
    canvas.selectAll('.normalRange rect').data(normalRanges).enter().insert('rect', ':first-child').attr({
        class: 'normalRange',
        x: function x(d) {
    ...
    .append('title').text(function (d) {
        return 'Normal range: ' + d.key.split(',')[0] + '-' + d.key.split(',')[1] + (chart.config.unit_col ? '' + chart.filtered_data[0][chart.config.unit_col] : '') + (' (' + d3$1.format('%')(d.values / chart.filtered_data.length) + ' of records)');
    });
}
...
function onResize() {
    ...
    if (config.normal_range) {
        ...
        normalRangeControl.on('change', function () {
            chart.config.displayNormalRange = d3$1.select(this).select('input').property('checked');
    ...
    if (this.highlightedBin) bins.attr('fill-opacity', function (d) {
