...
function setInitialMeasure() {
    ...
    }).start = this.config.start_value && this.measures.indexOf(this.config.start_value) > -1 ? this.config.start_value : this.measures[0];
}
...
function addXdomainResetButton() {
    ...
    //Add label.
    resetContainer.append('span').attr('class', 'wc-control-label').style('text-align', 'right').text('X-axis:');

    //Add button.
    resetContainer.append('button').text('Reset Limits').on('click', function () {
...
function setXdomain() {
    if (this.currentMeasure !== this.previousMeasure)
        // new measure
        this.config.x.domain = this.measure_domain;else if (this.config.x.domain[0] > this.config.x.domain[1])
        // invalid domain
        this.config.x.domain.reverse();else if (this.config.x.domain[0] === this.config.x.domain[1])
...
function setXaxisLabel() {
    this.config.x.label = this.currentMeasure + (this.config.unit_col && this.measure_data[0][this.config.unit_col] ? ' (' + this.measure_data[0][this.config.unit_col] + ')' : '');
}
...
function updateXaxisLimitControls() {
    ...
    this.controls.wrap.selectAll('.control-group').filter(function (f) {
        return f.option === 'x.domain[0]';
    }).select('input').property('value', this.config.x.formatted_domain[0]);
    this.controls.wrap.selectAll('.control-group').filter(function (f) {
        return f.option === 'x.domain[1]';
    }).select('input').property('value', this.config.x.formatted_domain[1]);
}
...
function addBinClickListener() {
    ...
    bins.style('cursor', 'pointer').on('click', function (d) {
        ...
        //Update footnote.
        if (footnote.classed('tableTitle') === false) footnote.text(d.values.raw.length + ' records with ' + (chart.filtered_data[0][config.measure_col] + ' values from ') + (chart.config.x.d3_format1(d.rangeLow) + ' to ' + chart.config.x.d3_format1(d.rangeHigh)) + (config.unit_col ? ' ' + chart.filtered_data[0][config.unit_col] : ''));
...
function maintainBinHighlighting() {
    ...
    if (this.highlightedBin) this.svg.selectAll('.bar').attr('fill-opacity', function (d) {
