(function (global, factory) {
    ...
    function annotateBinBoundaries() {
        ...
        var binBoundaries = d3.set(d3.merge(this.current_data.map(function (d) {
            return [_this.config.x.d3format(d.rangeLow), _this.config.x.d3format(d.rangeHigh)];
        }))).values().sort(function (a, b) {
            return +a - +b;
        });
        this.svg.selectAll('text.bin-boundary').data(binBoundaries).enter().append('text').classed('bin-boundary', true).attr({
            x: function x(d) {
                return _this.x(+d);
            },
            y: this.y(0),
