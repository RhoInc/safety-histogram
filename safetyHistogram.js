(function (global, factory) {
    ...
    function styles() {
      var styles = ['.safety-histogram {' + '    display: inline-block;' + '    width: 100%;' + '}', '.safety-histogram > * {' + '    display: inline-block;' + '    width: 100%;' + '}', '.sh-controls {' + '}', '.sh-chart,' + '.sh-listing {' + '    width: 75%;' + '    float: left;' + '}', '.sh-chart {' + '}', '.sh-multiples {' + '    width: 24%;' + '    float: right;' + '}', '.sh-multiples .wc-small-multiples .wc-chart {' + '    display: inline-block;' + '    width: 100%;' + '    position: relative;' + '}', '.sh-multiples .wc-small-multiples .wc-chart.wc-chart-title {' + '    position: absolute;' + '    top: 0;' + '    right: 0;' + '    width: 100%;' + '}', '.sh-statistical-test {' + '    position: absolute;' + '    top: 0;' + '    right: 0;' + '    font-weight: bold;' + '    cursor: help;' + '}', '#group-by {' + '    width: 100%;' + '    padding-bottom: 4px;' + '    border-bottom: 1px solid #aaa;' + '    margin-bottom: 4px;' + '    display: flex;' + '    justify-content: center;' + '}', '#group-by .wc-control-label {' + '    margin-right: 5px;' + '}', '#group-by .span-description {' + '    display: none;' + '}', '#group-by .changer {' + '}', '.sh-listing {' + '}']; // Attach styles to DOM.
    ...
    function identifyControls() {
      var context = this;
      var controlGroups = this.controls.wrap.style('padding-bottom', '8px').selectAll('.control-group'); // Give each control a unique ID.
    ...
    function addFootnoteContainer() {
      ...
      this.footnotes.barDetails = this.footnotes.container.append('p').classed('footnote footnote--bar-details', true).html('<br>');
    }
    ...
    function updateBinAlogrithm() {
      this.controls.Algorithm.selectAll('.algorithm-explanation').style('display', this.config.x.bin_algorithm !== 'Custom' ? null : 'none').attr('title', this.config.x.bin_algorithm !== 'Custom' ? "View information on ".concat(this.config.x.bin_algorithm) : null);
    }
    ...
    function resetRenderer() {
      ...
      this.svg.selectAll('.bar-group').classed('selected', false).selectAll('.bar').attr('fill-opacity', 0.75); // Reset footnotes.
    ...
    function runShapiroWilkTest() {
      ...
      var x = new jerzy.Vector(this.raw_data.map(function (d) {
      ...
      this.wrap.insert('span', ':first-child').classed('sh-statistical-test sh-sw-test', true).attr('title', 'The Shapiro-Wilk test tests the null hypothesis that a sample x[1], ..., x[n] came from a normally distributed population.').text("p = ".concat(d3.format('.2f')(this.sw.p)));
    }
    ...
    function runKolmogorovSmirnovTest() {
      ...
      var y = new jerzy.Vector(this.filtered_data.map(function (d) {
        return +d[_this.config.x.column];
      }));
      this.ks = jerzy.Nonparametric.kolmogorovSmirnov(x, y); // Annotate p-value.

      this.wrap.select('.wc-chart-title').append('span').classed('sh-statistical-test sh-ks-test', true).attr('title', 'The Kolmogorov-Smirnov test tests whether two underlying one-dimensional probability distributions differ.').text("p = ".concat(d3.format('.2f')(this.ks.p)));
    }
    ...
    function addHoverBars() {
      ...
      var bins = this.svg.selectAll('.bar-group').each(function (d) {
        ...
        d.footnote = "<span style = 'font-weight: bold'>".concat(d.values.raw.length, " records</span> with ") + "".concat(context.measure.current, " values &ge;<span style = 'font-weight: bold'>") + "".concat(context.config.x.d3format1(d.rangeLow), "</span> and ").concat(d.rangeHigh < context.config.x.domain[1] ? '&lt;' : '&le;', "<span style = 'font-weight: bold'>").concat(context.config.x.d3format1(d.rangeHigh), "</span>");
      });
    }

    function mouseout(element, d) {
      // Update footnote.
      this.footnotes.barDetails.html(this.highlightedBin ? "Table displays ".concat(this.highlighteD.footnote, ".") : '<br>'); // Remove bar highlight.
    ...
    function mouseover(element, d) {
      ...
      this.multiples.multiples.forEach(function (multiple) {
        multiple.marks[0].groups.each(function (di) {
          if (di.key === d.key) console.log(this.remove());
        });
      });
    }
    ...
    function deselect(element, d) {
      ...
      this.footnotes.barDetails.text("".concat(d.values.raw.length, " records with ") + "".concat(this.measure.current, " values from ") + "".concat(this.config.x.d3format1(d.rangeLow), " to ").concat(this.config.x.d3format1(d.rangeHigh)));
    }
    ...
    function safetyHistogram() {
      ...
      }, syncedSettings); // Define controls.
