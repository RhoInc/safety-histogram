(function (global, factory) {
    ...
    function calculateStatistics(obj) {
        ...
        obj.stats.calculatedBins = obj.stats.calculatedBinWidth > 0 ? Math.ceil(obj.stats.range / obj.stats.calculatedBinWidth) : NaN;
        obj.stats.nBins = obj.stats.calculatedBins < obj.stats.nUnique ? obj.stats.calculatedBins : obj.stats.nUnique;
    ...
    function drawNormalRanges() {
        ...
        if (this.config.displayNormalRange) {
            ...
            .rollup(function (d) {
                ...
                //tooltip
                d.tooltip = 'Normal range: ' + d.lower + ' - ' + d.upper + ' (' + d3.format('%')(d.values / _this.filtered_data.length) + ' of records)';
            ...
            var rects = this.svg.selectAll('rect.normal-range').data(normalRanges).enter().insert('rect', '.bar-supergroup').classed('normal-range', true).attr({
                x: function x(d) {
                    return d.x1;
                },
                y: 0,
    ...
    function annotateBinBoundaries() {
        ...
        var binBoundaries = d3.set(d3.merge(this.current_data.map(function (d) {
        ...
        this.svg.append('g').classed('bin-boundaries', true).selectAll('text.bin-boundary').data(binBoundaries).enter().append('text').classed('bin-boundary', true).attr({
            x: function x(d) {
