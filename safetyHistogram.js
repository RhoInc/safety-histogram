(function (global, factory) {
    ...
    (global = global || self, global.safetyHistogram = factory(global.d3, global.webCharts));
    ...
    function syncSettings(settings) {
      ...
      if (!(settings.filters instanceof Array)) settings.filters = typeof settings.filters === 'string' ? [settings.filters] : []; // Handle a string argument to groups.

      if (!(settings.groups instanceof Array)) settings.groups = typeof settings.groups === 'string' ? [settings.groups] : []; // stratification
      ...
      if (!(settings.groups instanceof Array && settings.groups.length)) {
        ...
        if (settings.group_by) settings.groups.push({
          value_col: settings.group_by,
          label: settings.group_by
        });
      } else settings.groups = [defaultGroup].concat(settings.groups.map(function (group) {
        return {
      ...
      settings.groups = d3$1.set(settings.groups.map(function (group) {
      ...
      settings.group_label = settings.group_by ? settings.groups.find(function (group) {
        return group.value_col === settings.group_by;
      }).label : settings.groups.length > 1 ? settings.groups[1].label : defaultGroup.label; // handle a string argument to details

      if (!(settings.details instanceof Array)) settings.details = typeof settings.details === 'string' ? [settings.details] : []; // Define default details.
      ...
      if (settings.normal_col_low) defaultDetails.push({
      ...
      if (settings.hasOwnProperty('displayNormalRange')) settings.display_normal_range = settings.displayNormalRange; // Update normal range settings if normal_range is set to false.
    ...
    function controlInputs() {
      return [{
        ...
        values: ['Square-root choice', "Sturges' formula", 'Rice Rule', // 'Doane\'s formula',
    ...
    function layout(element) {
      var containers = {
        main: d3$1.select(element).append('div').classed('safety-histogram', true)
      };
    ...
    function styles(settings) {
      var styles = ['.safety-histogram {' + '    display: inline-block;' + '    width: 100%;' + '}', '.safety-histogram > * {' + '    display: inline-block;' + '    width: 100%;' + '}', '.sh-hidden {' + '    display: none !important;' + '}',
      /***--------------------------------------------------------------------------------------\
        Controls
      \--------------------------------------------------------------------------------------***/
      '.sh-controls {' + '}', '.sh-controls .wc-controls {' + '    border-bottom: 1px solid #ccc;' + '    padding-bottom: 20px;' + '    position: relative;' + '}', '.sh-algorithm-explanation {' + '    cursor: pointer;' + '}', '.sh-head-note {' + '    position: absolute;' + '    font-style: italic;' + '    bottom: 0px;' + '}', '.sh-head-note--participant-count {' + '    left: 0;' + '}', '.sh-head-note--removed-records {' + '    right: 0;' + '}', '.sh-clear-note {' + '    color: blue;' + '    text-decoration: underline;' + '    font-style: normal;' + '    font-weight: bold;' + '    cursor: pointer;' + '    font-size: 16px;' + '    margin-left: 5px;' + '}',
      /***--------------------------------------------------------------------------------------\
        Chart
      \--------------------------------------------------------------------------------------***/
      '.sh-chart {' + (settings.draw_multiples ? '    width: 75%;' + '    float: left;' : '    width: 100%;') + '}', '.bar-group {' + '    cursor: pointer;' + '}', '.sh-footnotes {' + '    border-top: 1px solid #ccc;' + '    padding-top: 10px;' + '}',
      /***--------------------------------------------------------------------------------------\
        Small multiples
      \--------------------------------------------------------------------------------------***/
      '.sh-multiples {' + (settings.draw_multiples ? '    display: inline-block;' + '    width: 24%;' + '    float: right;' : '    display: none;') + '}', '.sh-multiples .wc-small-multiples .wc-chart {' + '    display: inline-block;' + '    width: 100%;' + '    position: relative;' + '    padding: 0;' + '}', '.sh-multiples .wc-small-multiples .wc-chart .wc-chart-title {' + '    text-align: left;' + '    font-size: 12px;' + '}', '.sh-group-percentage {' + '    font-weight: normal;' + '}', '.sh-statistical-test {' + '    position: absolute;' + '    top: 0;' + '    right: 0;' + '    background: white;' + '    border: 1px solid #aaa;' + '    border-radius: 4px;' + '    padding: 2px 4px;' + '}', '.sh-statistical-test__p-value {' + '    font-weight: bold;' + '    cursor: help;' + '}', '.sh-statistical-test__info {' + '    font-weight: normal;' + '    cursor: pointer;' + '}', '#group-by {' + '    width: 100%;' + '    padding-bottom: 4px;' + '    border-bottom: 1px solid #aaa;' + '    margin-bottom: 4px;' + '    display: flex;' + '    justify-content: center;' + '}', '#group-by .wc-control-label {' + '    margin-right: 5px;' + '}', '#group-by .span-description {' + '    display: none;' + '}', '#group-by .changer {' + '}',
      /***--------------------------------------------------------------------------------------\
        Listing
      \--------------------------------------------------------------------------------------***/
      '.sh-listing {' + (settings.draw_multiples ? '    width: 75%;' + '    float: left;' : '    width: 100%;') + '}']; // Attach styles to DOM.
    ...
    function checkRequired() {
      ...
      this.variables.required.forEach(function (definition) {
        if (_this.variables.actual.indexOf(definition.setting) < 0) {
          ...
          var errorText = "The variable specified for <code style='".concat(codeStyle.join(';'), "'>").concat(definition.property, "</code>, <em>").concat(definition.setting, "</em>, does not exist in the data."); // Print error to console.
      ...
      this.variables.optional.forEach(function (definition) {
        if (definition.type === 'string') {
          if (_this.variables.actual.indexOf(definition.setting) < 0) {
            ...
            console.warn("The variable specified for [ ".concat(definition.property, " ], ").concat(definition.setting, ", does not exist in the data."));
          }
            ...
            definition.setting.forEach(function (subDefinition, i) {
              ...
              if (_this.variables.actual.indexOf(variable) < 0) {
                definition.missing = true;
                console.warn("The variable specified for [ ".concat(definition.property, "[").concat(i, "] ], ").concat(variable, ", does not exist in the data."));
              }
            });
    ...
    function removeMissingResults() {
      ...
      if (this.removedRecords.placeholderRecords.length) console.log("".concat(this.removedRecords.placeholderRecords.length, " participants without results have been detected.")); // Count the number of records with missing results.
      ...
      if (this.removedRecords.missing > 0) console.warn("".concat(this.removedRecords.missing, " record").concat(this.removedRecords.missing > 1 ? 's with a missing result have' : ' with a missing result has', " been removed.")); // Update data.
    ...
    function removeNonNumericResults() {
      ...
      if (this.removedRecords.nonNumeric > 0) console.warn("".concat(this.removedRecords.nonNumeric, " record").concat(this.removedRecords.nonNumeric > 1 ? 's with a non-numeric result have' : ' with a non-numeric result has', " been removed.")); // Update data.
    ...
    function addVariables() {
      ...
      this.raw_data.forEach(function (d) {
        ...
        d.sh_measure = d.hasOwnProperty(_this.config.unit_col) ? "".concat(d[_this.config.measure_col], " (").concat(d[_this.config.unit_col], ")") : d[_this.config.measure_col]; // Coerce y-variable to numeric.
    ...
    function updateMeasureFilter() {
      ...
      if (this.config.start_value && this.sh_measures.indexOf(this.config.start_value) < 0 && this.measures.indexOf(this.config.start_value) < 0) {
        ...
        console.warn("".concat(this.config.start_value, " is an invalid measure. Defaulting to ").concat(measureInput.start, "."));
        ...
        console.warn("".concat(this.config.start_value, " is missing the units value. Defaulting to ").concat(measureInput.start, "."));
    ...
    function addXdomainResetButton() {
      ...
      this.controls.reset.label = this.controls.reset.container.append('span').attr('class', 'wc-control-label').text(''); // Add button.
    ...
    function insertGrouping(selector, label) {
      ...
      var div = this.controls.wrap.insert('div', selector).classed("".concat(className, "-div"), true).style({
        display: 'inline-block',
        'margin-right': '5px'
      });
      var fieldset = div.append('fieldset').classed("".concat(className, "-fieldset"), true).style('padding', '0px 2px');
      var legend = fieldset.append('legend').classed("".concat(className, "-legend"), true).text(label);
    ...
    function groupControls() {
      ...
      if (this.filters.length > 1) insertGrouping.call(this, '.subsetter:not(#measure)', 'Filters'); // Group bin controls.
    ...
    function addXdomainZoomButton() {
      ...
      if (this.filters.find(function (filter) {
        ...
        resetContainer.append('span').attr('class', 'wc-control-label').text(''); // Add button.
    ...
    function customizeBinsEventListener() {
      ...
      var context = this;
      this.controls.Algorithm.selectAll('.wc-control-label').append('span').classed('sh-algorithm-explanation', true).html(' &#9432').on('click', function () {
        if (_this.config.x.bin_algorithm !== 'Custom') window.open("https://en.wikipedia.org/wiki/Histogram#".concat(_this.config.x.bin_algorithm.replace(/ /g, '_').replace('Freedman-Diaconis', 'Freedman%E2%80%93Diaconis')));
      });
    ...
    function addParticipantCountContainer() {
      this.participantCount.container = this.controls.wrap.append('div').classed('sh-head-note sh-head-note--participant-count', true).style({
    ...
    function addRemovedRecordsNote() {
      ...
      if (this.removedRecords.missing > 0 || this.removedRecords.nonNumeric > 0) {
        var message = this.removedRecords.missing > 0 && this.removedRecords.nonNumeric > 0 ? "".concat(this.removedRecords.missing, " record").concat(this.removedRecords.missing > 1 ? 's' : '', " with a missing result and ").concat(this.removedRecords.nonNumeric, " record").concat(this.removedRecords.nonNumeric > 1 ? 's' : '', " with a non-numeric result were removed.") : this.removedRecords.missing > 0 ? "".concat(this.removedRecords.missing, " record").concat(this.removedRecords.missing > 1 ? 's' : '', " with a missing result ").concat(this.removedRecords.missing > 1 ? 'were' : 'was', " removed.") : this.removedRecords.nonNumeric > 0 ? "".concat(this.removedRecords.nonNumeric, " record").concat(this.removedRecords.nonNumeric > 1 ? 's' : '', " with a non-numeric result ").concat(this.removedRecords.nonNumeric > 1 ? 'were' : 'was', " removed.") : '';
        this.removedRecords.container = this.controls.wrap.append('div').classed('sh-head-note sh-head-note--removed-records', true).text(message);
        this.removedRecords.container.append('span').classed('sh-clear-note', true).html('<sup>x</sup>').on('click', function () {
    ...
    function addFootnoteContainer() {
      ...
      this.footnotes.barClick = this.footnotes.container.append('p').classed('sh-foot-note sh-foot-note--bar-click', true).text('Click a bar for details.');
      this.footnotes.barDetails = this.footnotes.container.append('p').classed('sh-foot-note sh-foot-note--bar-details', true).html('<br>');
    }
    ...
    function setXdomain() {
      if (this.measure.current !== this.measure.previous) this.config.x.domain = this.measure.raw.domain;else if (this.config.x.domain[0] > this.config.x.domain[1]) this.config.x.domain.reverse(); // The x-domain can be in three states:
      ...
      this.measure.domain_state = this.config.x.domain[0] === this.measure.raw.domain[0] && this.config.x.domain[1] === this.measure.raw.domain[1] || this.measure.previous === undefined ? 'raw' : 'custom'; // Set chart data to measure data.
    ...
    function calculateXPrecision() {
      ...
      this.config.x.format = this.config.x.precisionFactor > 0 ? '.0f' : ".".concat(Math.abs(this.config.x.precisionFactor) + 1, "f");
      this.config.x.d3format = d3$1.format(this.config.x.format); // one more precision please: bin format

      this.config.x.format1 = this.config.x.precisionFactor > 0 ? '.1f' : ".".concat(Math.abs(this.config.x.precisionFactor) + 2, "f");
    ...
    function updateBinAlogrithm() {
      this.controls.Algorithm.selectAll('.sh-algorithm-explanation').classed('sh-hidden', this.config.x.bin_algorithm === 'Custom').attr('title', this.config.x.bin_algorithm !== 'Custom' ? "Click to view information on ".concat(this.config.x.bin_algorithm, ".") : null);
    }
    ...
    function updateParticipantCount() {
      ...
      this.participantCount.percentage = d3$1.format('0.1%')(this.participantCount.n / this.participantCount.N); // clear the annotation

      this.participantCount.container.selectAll('*').remove(); // update the annotation

      this.participantCount.container.text("\n".concat(this.participantCount.n, " of ").concat(this.participantCount.N, " participant(s) shown (").concat(this.participantCount.percentage, ")"));
    }
    ...
    function resetRenderer() {
      ...
      if (this.config.draw_multiples && this.multiples && this.multiples.multiples) {
        this.multiples.multiples.forEach(function (multiple) {
          multiple.svg.selectAll('.bar-group').classed('selected', false).selectAll('.bar').attr('fill-opacity', 0.75);
        });
      } // Reset footnotes.
    ...
    Numeric._adaptive = function (f, a, b, eps, s, fa, fb, fc, depth) {
      ...
      if (depth <= 0 || Math.abs(s2 - s) <= 15 * eps) {
        ...
        return this._adaptive(f, a, c, eps / 2, left, fa, fc, fd, depth - 1) + this._adaptive(f, c, b, eps / 2, right, fc, fb, fe, depth - 1);
      }
    };
    ...
    Normal.prototype._de = function (x) {
      return 1 / (Math.sqrt(this.variance) * Math.sqrt(2 * Math.PI)) * Math.exp(-Math.pow(x - this.mean, 2) / (2 * this.variance));
    };
    ...
    T.prototype._de = function (x) {
      return Misc.gamma((this.df + 1) / 2) / (Math.sqrt(this.df * Math.PI) * Misc.gamma(this.df / 2)) * Math.pow(1 + Math.pow(x, 2) / this.df, -(this.df + 1) / 2);
    };
    ...
    function runShapiroWilkTest() {
      ...
      if (this.config.test_normality) {
        var x = new vector.Vector(this.raw_data.map(function (d) {
          return +d[_this.config.x.column];
        }));
        this.sw = Normality.shapiroWilk(x); // Annotate p-value.

        this.wrap.select('span.sh-sw-test').remove();
        var pValue = this.wrap.insert('span', ':first-child').classed('sh-statistical-test sh-sw-test', true);
        pValue.append('span').classed('sh-statistical-test__label', true).text('Normality: ');
        pValue.append('span').classed('sh-statistical-test__p-value', true).attr('title', 'The Shapiro-Wilk test tests the null hypothesis that a sample x[1], ..., x[n] came from a normally distributed population.').text("p = ".concat(d3.format('.2f')(this.sw.p)));
        pValue.append('span').classed('sh-statistical-test__info', true).attr('title', 'Click to view information on the Shapiro-Wilkd test.').html(' &#9432').on('click', function () {
    ...
    function runKolmogorovSmirnovTest() {
      ...
      if (this.config.compare_distributions && this.config.group_by !== 'sh_none') {
        var allResults = new vector.Vector(this.raw_data.map(function (d) {
          return +d[_this.config.x.column];
        }));
        this.groups = d3$1.set(this.initial_data.map(function (d) {
          return d[_this.config.group_by];
        })).values().map(function (value) {
          var group = {
            value: value
          };
          group.results = new vector.Vector(_this.raw_data.filter(function (d) {
    ...
    function annotatePercentage() {
      this.wrap.select('.wc-chart-title').append('span').classed('sh-group-percentage', true).text(" (".concat(d3.format('%')(this.filtered_data.length / this.raw_data.length), " of records)"));
    }
    ...
    function annotatePValues() {
      ...
      if (this.sh.config.compare_distributions) {
        ...
        var pValue = this.wrap.select('.wc-chart-title').append('span').classed('sh-statistical-test sh-ks-test', true);
        pValue.append('span').classed('sh-statistical-test__p-value', true).attr('title', 'The two-sample Kolmogorov-Smirnov test tests whether two underlying one-dimensional probability distributions differ.\n' + "In this case the test compares the distribution of results where ".concat(this.sh.config.group_label, " = ").concat(this.filters[0].val, " with that of the full set of results.")).text("p = ".concat(d3.format('.2f')(this.ks.p)));
        pValue.append('span').classed('sh-statistical-test__info', true).attr('title', 'Click to view information on the two-sample Kolmogorov-Smirnov test.').html(' &#9432').on('click', function () {
          window.open('https://en.wikipedia.org/wiki/Kolmogorov%E2%80%93Smirnov_test#Two-sample_Kolmogorov%E2%80%93Smirnov_test');
        });
      }
    }
    ...
    function addHoverBars() {
      ...
      var bins = this.svg.selectAll('.bar-group').each(function (d) {
        ...
        var hoverBar = g.insert('path', ':first-child').classed('hover-bar', true).attr({
          d: "M ".concat(x, " ").concat(y, " V ").concat(height, " H ").concat(x + width, " V ").concat(y),
          fill: 'black',
          'fill-opacity': 0,
          stroke: 'black',
          'stroke-opacity': 0
        });
        d.footnote = "<span style = 'font-weight: bold'>".concat(d.values.raw.length, " records</span> where ").concat(context.sh ? "<span style = 'font-weight: bold'>".concat(safetyHistogram.config.group_label, " = ").concat(context.filters[0].val, "</span> and") : '', " ").concat(safetyHistogram.measure.current, " is &ge;<span style = 'font-weight: bold'>").concat(safetyHistogram.config.x.d3format1(d.rangeLow), "</span> and ").concat(d.rangeHigh < context.config.x.domain[1] ? '&lt;' : '&le;', "<span style = 'font-weight: bold'>").concat(safetyHistogram.config.x.d3format1(d.rangeHigh), "</span>");
      });
    }
    ...
    function mouseout(element, d) {
      ...
      safetyHistogram.footnotes.barDetails.html(safetyHistogram.highlightedBin ? "Table displays ".concat(safetyHistogram.highlighteD.footnote, ".") : '<br>'); // Remove bar highlight.
      ...
      if (this.config.draw_multiples) {
        var otherCharts = this.multiples ? this.multiples.multiples : this.parent.multiples.filter(function (multiple) {
        ...
        otherCharts.forEach(function (chart) {
          chart.marks[0].groups.each(function (di) {
            if (di.key === d.key) d3$1.select(this).selectAll('.bar').attr('stroke', context.colorScale());
          });
        });
      }
    }
    ...
    function select(element, d) {
      ...
      if (safetyHistogram.config.draw_multiples && safetyHistogram.multiples && safetyHistogram.multiples.multiples) {
        safetyHistogram.multiples.multiples.forEach(function (multiple) {
          multiple.svg.selectAll('.bar-group').selectAll('.bar').attr('fill-opacity', 0.5);
        });
      }

      d3$1.select(element).select('.bar').attr('fill-opacity', 1); // Update bar click footnote

      safetyHistogram.footnotes.barClick.style({
        cursor: 'pointer',
    ...
    function deselect(element, d) {
      ...
      safetyHistogram.footnotes.barDetails.text("".concat(d.values.raw.length, " records with ") + "".concat(safetyHistogram.measure.current, " values from ") + "".concat(safetyHistogram.config.x.d3format1(d.rangeLow), " to ").concat(safetyHistogram.config.x.d3format1(d.rangeHigh)));
    }
    ...
    function drawNormalRanges() {
      ...
      if (this.config.display_normal_range && this.filtered_data.length > 0) {
        ...
        var normalRanges = d3$1.nest().key(function (d) {
          return "".concat(d[_this.config.normal_col_low], ",").concat(d[_this.config.normal_col_high]);
        ...
        .rollup(function (d) {
          ...
          d.tooltip = d.values < _this.filtered_data.length ? "".concat(d.lower, " - ").concat(d.upper, " (").concat(d3$1.format('%')(d.rate), " of records)") : "".concat(d.lower, " - ").concat(d.upper); // plot if:
        ...
        this.controls.wrap.selectAll('#normal-range .wc-control-label').append('span').classed('normal-range-list', true).html(' &#9432').attr('title', normalRanges.length > 1 ? "".concat(this.measure.current, " normal ranges:\n").concat(normalRanges.map(function (normalRange) {
          return normalRange.tooltip;
        }).join('\n')) : "".concat(this.measure.current, " normal range: ").concat(normalRanges[0].tooltip)).style('cursor', 'default'); // Add groups in which to draw normal range rectangles and annotations.
        ...
        var groups = group.selectAll('g.normal-range').data(normalRanges.filter(function (d) {
          return d.plot;
        })).enter().append('g').classed('normal-range', true); // Draw normal range rectangles.
    ...
    function safetyHistogram() {
      ...
      }, syncedSettings); // Manipulate DOM and define a style sheet.
