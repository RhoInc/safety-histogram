    ...
    if (groupControl.datum().values.length === 1) groupControl.style('display', 'none');
        ...
        const select = groupControl
            ...
            .on('change', function(d) {
                ...
                context.config.group_by = context.config.groups
                    .find(group => group.label === this.value)
                    .value_col;
        ...
        const options = select
            .selectAll('option')
            .property('selected', d => this.config.groups.find(group => group.label === d).value_col === this.config.group_by)
